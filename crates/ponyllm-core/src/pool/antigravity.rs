...
impl AntigravityTokenManager {
    ...
    pub async fn fetch_quota_summary(&self, endpoint: Option<&str>) -> Option<Vec<QuotaSummaryGroup>> {
        ...
        // Fallback endpoints for retrieveUserQuotaSummary.
        // Put production/daily cloudcode endpoints first to prevent sandbox timeouts.
        let mut endpoints = Vec::new();
        if let Some(ep) = endpoint {
            endpoints.push(ep.trim_end_matches('/').to_string());
        }
        ...
        for cand in fallback_candidates {
            if !endpoints.iter().any(|e| e == cand) {
                endpoints.push(cand.to_string());
            }
        }
        ...
        for base in endpoints {
            ...
            let Some(groups_arr) = json_val.get("groups").and_then(|v| v.as_array()) else {
                continue;
            };
